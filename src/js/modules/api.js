import { VISUAL_CROSSING_API_KEY, VISUAL_CROSSING_BASE_URL } from '../utils/constants';
...
export default async function fetchWeatherData(location) {
  ...
  const url = new URL(`${VISUAL_CROSSING_BASE_URL}${encodeURIComponent(location.trim())}`);
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Failed to fetch weather data: ${response.status} - ${errorData}`);
    }
